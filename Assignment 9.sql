...
-- Validate column 'description' is BLOB
UPDATE actor SET description = 123 WHERE first_name = 'GROUCHO';
...
SELECT CONCAT(s.first_name,' ', s.last_name) AS 'Staff Name', SUM(p.amount) AS 'Total Amount'
FROM staff s
JOIN payment p
...
SELECT f.title AS 'Film Title', COUNT(a.actor_id) AS 'No. of Actors'
FROM film f
...
SELECT f.title AS 'Film Title', COUNT(i.inventory_id) AS 'Copies Available'
FROM film f
...
-- Alternate method by concatting the Customer Name.
...
SELECT fl.title AS 'Film Title', ct.name AS 'Category'
FROM film fl, film_category fc, category ct
WHERE fl.film_id = fc.film_id
...
SELECT f.title AS 'Film Title', COUNT(r.rental_id) AS 'Rental Frequency'
FROM film f
JOIN inventory i
ON f.film_id = i.film_id
JOIN rental r
...
/* Determining and Validating Solution
SELECT film_id FROM film WHERE title = 'BUCKET BROTHERHOOD';

SELECT inventory_id FROM inventory WHERE film_id = 103;

SELECT COUNT(rental_id) FROM rental WHERE inventory_id > 464 AND inventory_id < 473;
...
SELECT s.store_id, SUM(p.amount) AS 'Amount in Dollar'
FROM store s
JOIN inventory i
...
CREATE OR REPLACE VIEW top_five_genres_by_revenue AS
SELECT ct.name AS 'Genres', SUM(p.amount) AS 'Gross Revenue'
FROM  category ct
JOIN film_category fc
ON fc.category_id = ct.category_id
